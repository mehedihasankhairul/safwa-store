  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        ...
        {/* Name + Chevron */}
        <span className={`text-sm font-bold max-w-[100px] truncate hidden sm:inline ${variant === "dark" ? "text-white" : "text-gray-900"
          }`}>
          {user.name?.split(" ")[0]}
        </span>
      ...
      {/* Dropdown Menu */}
      {isOpen && (
        <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-xl shadow-xl border border-gray-200 py-2 z-[100] animate-in fade-in slide-in-from-top-2">
          {/* User Info Header */}
          <div className="px-4 py-3 border-b border-gray-100 bg-gray-50/50 rounded-t-xl">
            <p className="text-sm font-bold text-gray-900 truncate">{user.name}</p>
            <p className="text-xs font-medium text-gray-500 truncate">{user.email || "Customer"}</p>
          </div>
          ...
          <div className="py-1">
            <Link
              ...
              className="flex items-center gap-3 px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-red-50 hover:text-red-800 transition-colors"
            >
